#!/bin/bash
# Online-style profiling for nano-vllm with periodic flushing to capture
# warmup and steady-state phases separately.
#
# Simulates online serving by sending sequential requests with varying prompt
# lengths through nano-vllm. Uses Proton's periodic flushing mode to produce
# per-phase .part_N.hatchet files for warmup vs steady-state comparison.
#
# Usage: bash scripts/profiling/nano_vllm_online.sh [OPTIONS]
...
OUTPUT_BASE="$REPO_ROOT/profiling_output/nano_vllm/online"
...
echo "============================================"
echo "nano-vllm Online Profiling (Periodic Flushing)"
echo "============================================"
echo "Model:           $MODEL"
...
echo "--------------------------------------------"
echo "Running online-style profiling with periodic flushing"
echo "  Context: $CONTEXT, Data: $DATA"
echo "  Output: $CONFIG_DIR"
echo "--------------------------------------------"

if python "$SCRIPT_DIR/helpers/nano_vllm_online_run.py" \
    --model "$MODEL" \
