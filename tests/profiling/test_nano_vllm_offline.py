"""Tests for the nano-vllm offline batch profiling script.
...
SCRIPT_PATH = os.path.join(SCRIPT_DIR, "nano_vllm_offline.sh")


class TestNanoVllmOfflineScript:
    """Verify nano_vllm_offline.sh structure and content."""
    ...
    def test_four_configs_total(self):
        ...
        # Count entries in CONFIGS array
        config_entries = [
            line.strip()
    ...
    def test_output_base_dir(self):
        """Output base is profiling_output/nano_vllm/offline/."""
        assert "profiling_output/nano_vllm/offline" in self.source
    ...
    def test_uses_triton_hook(self):
        """Uses --hook triton for Triton kernel metadata."""
    ...
    def test_calls_nano_vllm_helper(self):
        """Calls the nano_vllm_offline_run.py helper."""
    ...
    def test_prints_pass_fail(self):
        ...
        assert "Passed" in self.source or "PASS" in self.source
        assert "Failed" in self.source or "FAIL" in self.source
...
class TestNanoVllmOfflineSyntax:
    """Verify shell script has valid syntax."""

    def test_bash_syntax(self):
        """nano_vllm_offline.sh passes bash syntax check."""
